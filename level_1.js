...
let numbers = 12345
let arrNumbers = numbers.toString().split('').map(Number); // map(Number) — преобразует каждый символ обратно в число.
...
const arrayY = ['http://something.com.html', 'vk.html', 'http://somewhere.com', 'pochta']

const htmlArr = arrayY.filter(str => str.endsWith('.html')); // используем фильтр для поиска строк с нужным значением
